...
import { getManualEntry, manualEntries } from "../data"
...
import { ChevronRight, ArrowLeft, BookMarked, Clock } from "lucide-react"
...
export default async function ManualDetailPage({ params }: Props) {
  ...
  const doc = getManualContent(slug)
  const currentIndex = manualEntries.findIndex((d) => d.slug === slug)
  const prevDoc = currentIndex > 0 ? manualEntries[currentIndex - 1] : null
  const nextDoc = currentIndex < manualEntries.length - 1 ? manualEntries[currentIndex + 1] : null
    ...
    <>
      ...
      <div className="sticky top-14 z-30 border-b border-border bg-[var(--color-page)]/90 px-4 py-2 backdrop-blur-md lg:hidden">
        <nav className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
          <Link href="/manual" className="transition hover:text-foreground">
            使用手册
          </Link>
      ...
      <div className="mx-auto max-w-[1440px]">
        <div className="lg:grid lg:grid-cols-[280px_1fr_200px] lg:gap-0">
          ...
          <main className="min-w-0 px-4 py-8 md:px-8 md:py-10 lg:px-10">
            ...
            {/* 文章内容 */}
            {doc ? (
              <ManualArticle content={doc.content} />
            ) : (
              <div className="rounded-xl border border-border bg-[rgba(252,251,248,0.45)] p-8 text-center">
                <p className="text-sm text-muted-foreground">该章节内容正在编写中。</p>
              </div>
            )}
          ...
          {doc?.sections && doc.sections.length > 0 && (
            <div className="hidden lg:block">
              <div className="sticky top-16 h-[calc(100vh-64px)] overflow-y-auto border-l border-border px-4 py-6">
                <ManualTOC sections={doc.sections} />
              </div>
            </div>
          )}
        </div>
...
export function generateStaticParams() {
  return manualEntries.map((entry) => ({ slug: entry.slug }))
}
