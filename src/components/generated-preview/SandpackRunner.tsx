...
export function SandpackRunner({
  ...
  const sandboxKey = visibleFiles
    .map(
      (path) =>
        `${path}:${sandpackFiles[path]?.code.length ?? 0}:${sandpackFiles[path]?.code.charCodeAt(0) ?? 0}`,
    )
    .join("|");

  return (
    <div className="vf-sandpack-runner h-full min-h-0">
      <SandpackProvider
        key={sandboxKey}
        template="vite-react-ts"
        customSetup={sandpackGeneratedAppDependencies}
        files={sandpackFiles}
        options={{
          autorun: true,
      ...
      >
        <SandpackLayout className="!h-full !border-0 !bg-transparent">
          <div className="flex h-full min-h-0 w-full flex-col bg-background">
            ...
            <div
              className={
                activeTab === "preview" ? "flex min-h-0 flex-1" : "hidden"
              }
            >
              <SandpackPreview
                showNavigator
                showOpenInCodeSandbox={false}
                className="vf-sandpack-preview !h-full !min-h-0 !border-0 !bg-white"
